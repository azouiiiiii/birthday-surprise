...
/* ================== Gift Ribbon Texture ================== */
function createGiftTexture(boxColor: string) {
  const size = 512;
  const canvas = document.createElement("canvas");
  ...
  /* 礼带参数 */
  const ribbonColor = "#0a8f3c"; // 高对比绿色
  const edgeColor = "#e8ffe8";   // 浅色描边
  const ribbonWidth = size * 0.18;
  const center = size / 2;

  /* 竖带 */
  ctx.fillStyle = ribbonColor;
  ctx.fillRect(center - ribbonWidth / 2, 0, ribbonWidth, size);

  /* 横带 */
  ctx.fillRect(0, center - ribbonWidth / 2, size, ribbonWidth);

  /* 描边，增强对比 */
  ctx.strokeStyle = edgeColor;
  ctx.lineWidth = 6;
  ctx.strokeRect(center - ribbonWidth / 2, 0, ribbonWidth, size);
  ctx.strokeRect(0, center - ribbonWidth / 2, size, ribbonWidth);
...
export default function Ornaments({
  ...
  const { scatterData, treeData } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (type === "gift") {
        y = Math.random() * 0.6;
        radius = 2.5 + Math.random() * (3.8 - 2.5);
      }

      if (type === "bauble") {
        y = 1.0 + Math.random() * 1.5;
        radius = Math.sqrt(Math.random()) * 3.6;
      }

      if (type === "bauble-small") {
  ...
  useFrame((state, delta) => {
    ...
    if (meshRef.current.userData.mix === undefined)
      meshRef.current.userData.mix = 0;
    ...
    const mix = meshRef.current.userData.mix;
    const time = state.clock.elapsedTime;
    ...
    for (let i = 0; i < count; i++) {
      ...
      const float = Math.sin(time + i) * 0.15 * (1 - mix);
      ...
      tempObject.rotation.x =
        scatterData[i].rotation.x * (1 - mix) +
        treeData[i].rotation.x * mix;

      tempObject.rotation.y =
        scatterData[i].rotation.y * (1 - mix) +
        treeData[i].rotation.y * mix;

      tempObject.scale.setScalar(
        scatterData[i].scale * (1 - mix) +
          treeData[i].scale * mix
      );
  ...
  const geometry = useMemo(() => {
    if (type === "gift") return new THREE.BoxGeometry(0.3, 0.3, 0.3);
    if (type === "bauble") return new THREE.SphereGeometry(0.15, 32, 32);
    return new THREE.SphereGeometry(0.08, 24, 24);
  }, [type]);

  const material = useMemo(() => {
    if (type === "gift") {
      const texture = createGiftTexture(color);
      return new THREE.MeshStandardMaterial({
        map: texture,
        roughness: 0.35,
        metalness: 0.15,
